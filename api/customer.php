<?php
	...
	function store(){
		...
		if(!empty($name)){
			...
			if ($stmt->rowCount()){
				...
				$sql = "INSERT INTO customers(name,address,phone_no) VALUES (:name,:address,:phone_no)";
				$stmt = $pdo->prepare($sql);
				$stmt ->bindParam(':name',$name);
	...
	function update($id){
		...
		if (!empty($name)){
			...
			$sql = "UPDATE customers SET name=:name,address=:address,phone_no=:phone_no WHERE id=:id";
			$stmt = $pdo->prepare($sql);
			$stmt -> bindParam(":name",$name);
			$stmt -> bindParam(":id",$id);
				$stmt ->bindParam(':address',$address);
